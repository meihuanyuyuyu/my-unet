...
class Myrandomrotation(object):
    def __call__(self,img:Tensor,label:Tensor):
        angel = random.uniform(0.1,2.1)
        return F.rotate(img,angel),F.rotate(label,angel)

class Myrandom_resize(object):
...
def visualization(predict:Tensor,label:Tensor,name)->None:
    size = predict.size()
...
def train(model:nn.Module,sampled_data:DataLoader,criterion:nn.Module,optimizer:optim.Optimizer,device:str='cuda'):
    ...
    for data in sampled_data:
        ...
        loss.backward()
        optimizer.step()
        f1 = metric.F1score_black_as_target(binarymap,labels.to(device))
        f1s.append(f1)
...
def  val(i,model:nn.Module,sampled_data:DataLoader,device:str='cuda'):
    ...
    for data in sampled_data:
        ...
        outputs =model(imgs.to(device))
        binarymap = torch.argmax(outputs,dim=1)
        f1score = metric.F1score_black_as_target(binarymap,labels.to(device))
        f1s.append(f1score)
        visualization(binarymap,labels,f'{i}val.png')
...
def save_result_json(fp:str,i,re,loss,val_f1,f1s):
    result = {'i': i, 'loss':loss,'f1':f1s, 'val_f1':val_f1}
    with open(os.path.join(fp,f'_{i}_{re}_train_score.json'),'w') as f:
        json.dump(result,f)
...
class Dice_loss(object):
    ...
    def _dice_loss(self,predict:Tensor,target:Tensor):
        target = torch.one_hot(target,num_classes = 2)
        dice = 2*(predict*target).sum()/(target.square().sum() + predict.square().sum())
        return 1- dice
