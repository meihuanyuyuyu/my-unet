...
def compare_data(model:nn.Module,dataset_path:str):
    pre_data = EM_mydataset(dataset_path,[Myrandom_resize([2000,2000]),Myrandomcrop([512,512]),Mynormalize()])
    ...
    for data in pre_sampled:
        ...
        pc = metric.precision(binarymap.to('cpu'),label,0)
        rc = metric.recall(binarymap.to('cpu'),label,0)
        f1 = metric.F1score_black_as_target(binarymap.to('cpu'),label)
        pcs.append(pc)
        rcs.append(rc)
        f1s.append(f1)
